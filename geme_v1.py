...
class FirstOrk(pygame.sprite.Sprite):
    def __init__(self, x, y):
        nom = str(random.choice(range(1, 4)))
        file_name = "Images/characters/enemies/ORK_" + nom + "/"
        self.sl_images = {"attack_right": [], "attack_left": [], "run_left": [], "run_right": [],
                          "die": []}
    ...
    def update(self, *args):
        if self.hp <= 0:
            if self.die_animation <= 18:
                self.image = self.sl_images["die"][self.die_animation // 3]
                self.die_animation += 1
        else:
            self.width = len(level[0])
            self.height = len(level)
            self.color = [[0 for i in range(self.width + 10)] for j in range(self.height + 10)]
            self.pred = [[(-1, -1) for i in range(self.width + 10)] for j in
...
class KnightWithSpear(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, hp=-1, damage=-1, armor=-1):
        super().__init__()
        ...
        if self.hp <= 0:
            ...
            if self.figth:
                ...
                if self.attack_animation == 10:
                    pygame.mixer.music.load(load_sound("knight_attak.mp3"))
                    pygame.mixer.music.play()
                ...
                if pygame.sprite.spritecollideany(self, group):
                    if self.attack_animation == 23:
                        ...
                        if sprite.hp <= 0:
                            ...
                            self.armor = min(self.armor + 20, self.max_armor)
                    else:
                        if self.armor - 3 >= 0:
                            sprite.armor -= 3
                        else:
                            sprite.hp -= 3 + sprite.armor
                            sprite.armor = 0
    ...
    def update(self, x, y, *args):
        if self.hp <= 0:
            ...
            if self.figth:
                ...
                self.pred_left = self.left
                self.attack_animation = self.attack_animation + 1
                if self.attack_animation == 3:
                    pygame.mixer.music.load(load_sound("rogue_attak.mp3"))
                    pygame.mixer.music.play()
                if self.attack_animation == 21:
                    self.figth = False
                    self.attack_animation = 0
...
class Dworf(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, hp=-1, damage=-1, armor=-1):
        ...
        if hp != None and hp > -1:
            ...
            self.max_hp, self.hp = 300, 300
            self.max_damage, self.damage = 25, 25
            self.max_armor, self.armor = 200, 200
        self.speed = 15
        self.alive = True
        self.go_hall = True
        self.left = False
    ...
    def update(self, x, y, *args):
        if self.hp <= 0:
            ...
            if self.figth:
                ...
                self.pred_left = self.left
                self.attack_animation = self.attack_animation + 1
                if self.attack_animation == 5:
                    pygame.mixer.music.load(load_sound("dworf_attak.mp3"))
                    pygame.mixer.music.play()
                if self.attack_animation == 24:
                    self.figth = False
                    self.attack_animation = 0
                ...
                if self.stop:
                    ...
                    if self.pred_left:
                        ...
                        self.image = self.sl_images["idle_right"][self.idle_animation // 3]
                    self.idle_animation = (self.idle_animation + 1) % 36
            for sprite in enemy_group:
                group = pygame.sprite.Group()
                sprite.add(group)
                if pygame.sprite.spritecollideany(self, group):
...
def start_screen():
    ...
    for line in intro_text.keys():
        text_rect_image = load_image("background\\text_fon_4.png", -1)
...
def registrate_windows():
    ...
    sprite.add(image)
    for line in intro_text.keys():
        text_rect_image = load_image("background\\text_fon_2.png", -1)
        pygame.draw.rect(text_rect_image, pygame.Color(255, 0, 0),
                         (0, 0, intro_text[line][1][0], intro_text[line][1][1]), 2)
        text_fon = pygame.sprite.Sprite()
        text_fon.image = text_rect_image
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if push_ok:
                    if normal_password2 != normal_password or len(normal_password) < 4:
                        ...
                        hero, level = get_result(login, normal_password, 1)
                        if hero and level:
                            text_sprite = pygame.sprite.Group()
...
def choise():
    global class_hero
    intro_text = {"KNIGHT": [(60, 540), (290, 240)], "DWORF": [(400, 325), (208, 200)],
                  "ROGUE": [(700, 485), (195, 200)]}
    sprite = pygame.sprite.Group()
    image = pygame.sprite.Sprite()
    ...
    sprite.add(image)
    for line in intro_text.keys():
        text_rect_image = load_image("background\\text_fon_3.png", -1)
        pygame.draw.rect(text_rect_image, pygame.Color(255, 0, 0),
                         (0, 0, intro_text[line][1][0], intro_text[line][1][1]), 2)
        text_fon = pygame.sprite.Sprite()
        text_fon.image = text_rect_image
...
def main():
    global all_sprites, tile_group, door_group, box_group, player_group, player_image, \
        class_hero, hall_group, enemy_group, level, player, info_group
    ...
    camera = Camera()
    hp_sprite = render_text(str(player.hp), 100, 50, color=(255, 0, 0))
                ...
                if push_continue:
                    ...
                    cur.execute("""UPDATE info_player SET hero = ?, hp = ?, damage = ?, armor = ?,  
                                   level = ? WHERE login = ?""", (hero, player.max_hp,
                                                                  player.max_damage,
                                                                  player.max_armor,
                                                                  number_level * 5 + number_room if ch != 0 else -1,
                                                                  player_login))
                    con.commit()
                    con.close()
                    return False
    ...
    for x in range(len(level)):
        for y in range(len(level[0])):
            if level[x][y] == ".":
                ...
                Tile("empty", x, y)
                if class_hero == "knight":
                    new_player = KnightWithSpear(x, y, hp, damage, armor)
                elif class_hero == "dworf":
                    new_player = Dworf(x, y, hp, damage, armor)
                elif class_hero == "rogue":
                    new_player = Rogue(x, y, hp, damage, armor)
    return new_player
